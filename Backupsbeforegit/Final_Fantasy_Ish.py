...
# character = {'Player Level': 1, 'Player Exp': 0, 'Next Level': 25}, 'player_stats', {'Strength': 1, 'Dexterity': 1, 'Intelligence': 1, 'attack' :[5, 12], 'health': 100}
    ...
    while character['player_stats']['health'] and monster_health >0:
        if "Rusty Sword" in player_inventory:
            character["player_stats"]['attack'] = (character["player_stats"]['attack'][0] + 5) , (character["player_stats"]['attack'][1] + 8)
        if "Mana Sword" in player_inventory and "shotgun ammo" in player_inventory:
...
def run_command(room, verb, noun):
    ...
    if verb in ["go", "g", "walk", "run", "sprint", "w", "r", "s"]:
        ...
        sys.exit()
            ...
            if room.room_name[0].value == 'Potos Inn':
                inn_system(room)
            return room
