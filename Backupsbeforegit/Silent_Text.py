...
def battle_system(room):
    # if "old key" in player_inventory:
    #     print 'cool'*200
        #if shotgun ammo count > 0 and shotgun in player inventory
            #player damage == 1000
        #if shotgun ammo count <=0 and shovel count in player inventory or shotgun not in player inventory and shovel in player inventory
        #player damage == 15
    pass
...
def run_command(room, verb, nouns):
    ...
    exits, items, locked, reqs = get_room_data(room)
...
def get_room_data(room):
    ...
    #     monster_dict[monster.attrs['name']] = monster.attrs['health']
    ...
    return exits, items , locked, pre_req_dict #,npcs
